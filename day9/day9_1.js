...
function extrapolate(sequence) {
	...
	let previousExtrapolatorValue = previousExtrapolator[previousExtrapolator.length - 1];
	for (let i = sequence.length - 2; i >= 0; i--) {
		let currentExtrapolator = sequence[i];
		let currentExtrapolatorValue = currentExtrapolator[currentExtrapolator.length - 1];
		currentExtrapolator.push(currentExtrapolatorValue + previousExtrapolatorValue);
